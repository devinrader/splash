...
- QUESTION: What concrete degraded-startup fallback source should `splash-protocol` use for pool-selected `protocol_plugin` and `protocol_config` when the normal configuration provider or PostgreSQL is unavailable?
